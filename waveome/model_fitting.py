...
# def random_restart_fit(X, Y, k, num_restarts):
#     models_out = Parallel(n_jobs=num_jobs, verbose=1)(delayed(kernel_test_reg)(
#         **locals)
...
def kernel_test_reg(X, Y, k, num_restarts=5, random_init=True,
                ...
                random_seed=None, num_inducing_points=500
                ):
    """
    ...
    for i in range(num_restarts):
                ...
                m = SVPGPR(
                    ...
                    inducing_variable=gpflow.inducing_variables.SeparateIndependentInducingVariables(
                        [gpflow.inducing_variables.InducingPoints(
                            ...
                            X_[np.random.randint(low=0, high=X.shape[0], size=num_inducing_points), :]
                            ) 
                    ...
                    #     gpflow.inducing_variables.InducingPoints(
                    #         X[np.random.randint(low=0, high=X.shape[0], size=num_inducing_points), :].copy()
                    #     )
                    # ),
        ...
        if lam > 0:
            if num_latent > 1:
                ...
                m.kernel.W.prior = tfd.Laplace(loc=f64(0), scale=f64(1./lam))
            else:
                ...
                for param_name, param_val in gpflow.utilities.parameter_dict(m).items():
                    if "kernel" in param_name and "variance" in param_name:
                        param_val.prior = tfd.Laplace(loc=f64(0), scale=f64(1./lam))
