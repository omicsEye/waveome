...
class Lin(gpflow.kernels.Kernel):
    def __init__(self, active_dims=None, variance=1.0): # , center=0.0):
...
def print_kernel_names2(kernel, with_idx=False):
    names = []
    ...
    if hasattr(kernel, 'kernels') == False:
        ...
        return [print_kernel_names2(x, with_idx) for x in kernel.kernels]
        # names.append('+'.join([print_kernel_names2(x) for x in kernel.kernels]))
    elif kernel.name == 'product':
        return '*'.join([print_kernel_names2(x, with_idx) for x in kernel.kernels])
    return names
...
def pred_kernel_parts(m, x_idx, unit_idx, col_names, 
                      lik='gaussian', x_idx_min = None, x_idx_max = None):
    """
    Breaks up kernel in model to plot separate pieces
    """
    ...
    # Get kernel names
    kernel_list = print_kernel_names2(m_copy.kernel, with_idx=True)
...
#                  y=mean.numpy().flatten(),
    ...
    if lik == 'gaussian':
        ...
        ax[plot_idx].scatter(m.data[0][:, x_idx],
                      resids,
                      color='black',
                      alpha=0.5,
                      s=20)
